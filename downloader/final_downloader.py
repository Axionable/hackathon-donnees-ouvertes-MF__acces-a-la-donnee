...
def collect_final_drias(donnees_json):
    liste_drias = next(
        (
            filtre["liste"]
            for filtre in donnees_json["filtreDonnees"]
            if filtre["nom"] == "Drias"
        ),
        None,
    )

    drias_data_netcdf = downloader_drias(
        donnees_json["DriasParams"]["scenario"],
        donnees_json["DriasParams"]["parametre"],
        donnees_json["DriasParams"]["modele"],
    )
...
def collect_final_data(donnees_json):
    bool_drias = any(
        filtre["nom"] == "Drias" for filtre in donnees_json["filtreDonnees"]
    )
